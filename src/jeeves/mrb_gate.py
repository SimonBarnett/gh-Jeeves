...
def build_check_run_payload(v: MrbVerdict) -> dict[str, Any]:
    ...
    # FAIL verdict with valid process still "succeeds" the gate (merge allowed only on PASS
    # via separate rule: only PASS conclusion allows merge — FAIL check fails merge)
    if ok and (v.verdict or "").strip().upper() == "FAIL":
        # Process-valid FAIL: check succeeds so history is recorded, but branch
        # protection should require success only when merging on PASS. Seats merge
        # on PASS themselves after posting success. For FAIL, conclusion=neutral
        # keeps main protected without greenwashing.
        conclusion = "neutral"
        errors = []  # already ok
    out = verdict_to_check_output(v, validate_verdict(v) if conclusion == "failure" else [])
    if conclusion == "neutral":
        out = verdict_to_check_output(v, [])
        out["summary"] = "MRB FAIL recorded (valid process; do not merge as PASS)"
    return {
        "name": CHECK_NAME,
        "head_sha": v.head_sha,
        "status": "completed",
        "conclusion": conclusion if validate_verdict(v) == [] or conclusion == "neutral" else "failure",
        "output": out,
    }

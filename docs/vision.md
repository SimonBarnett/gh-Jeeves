...
| S2 | CAST IRON: Jeeves owns !bored → assign (FR #106); no ear OFFER | Assign line on trusted !bored; 0 OFFER emits from Jeeves | `pytest tests/cast_iron/` + `tests/test_assign_fr106.py` | Ear OFFER returns; assign missing; multi-line claim wakes |
...
Jeeves service <--- drain outbox -----+
   ...
   | in every #{machine}: !bored -> assign; ACK/DONE -> accepted/busy, done/idle
   +--> POST /bob/v1/report (queue + worker state)

Workers !bored / ACK / DONE in their own #{machine} only (FR #106)
bob-{machine} ear stays on IRC for peer/digest; OFFER path retired
```
...
- (Resolved FR #106) Assign emitter: Jeeves one-line assign; ear OFFER retired

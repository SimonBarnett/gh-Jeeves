...
def test_fail_still_needs_min_duration():
    """Instant FAIL is also rejected — hostile floor applies to both verdicts."""
    v = MrbVerdict(
        pr=1,
        head_sha="abc",
        author_seat="a-1",
...
def test_build_check_run_payload_pass():
    from jeeves.mrb_gate import build_check_run_payload

    v = MrbVerdict(
        pr=100,
        head_sha="abc123",
        author_seat="flamingo-43052",
        reviewer_seat="marchhare-31712",
        verdict="PASS",
        pytest_cmd="python -m pytest tests/ -q",
        pytest_exit=0,
        duration_s=900,
    )
    body = build_check_run_payload(v)

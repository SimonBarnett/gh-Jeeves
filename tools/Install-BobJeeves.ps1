...
$DisableResyncFlag = [bool]$DisableResync
if ($cfg) {
    ...
    if ($cfg.PSObject.Properties.Name -contains 'disable_resync' -and $cfg.disable_resync -eq $true) {
        $DisableResyncFlag = $true
    }
    if ($cfg.tls -eq $true -and -not $NoTls) { $Tls = $true }
}
# Expand remaining env tokens (Apply as admin may still expand %USERPROFILE% of operator — document absolute paths for LocalSystem)
...
$plan = [ordered]@{
    ...
    disable_resync       = [bool]$DisableResyncFlag
    ...
    if ($plan.ok -and $plan.errors.Count -eq 0) {
        ...
        if (-not $plan.python) {
            ...
            if ($plan.ok) {
                ...
                if ($DisableResyncFlag) { $envExtra += "JEEVES_RESYNC_DISABLE=1" }
                & $plan.nssm set $ServiceName AppEnvironmentExtra $envExtra | Out-Null
                [void]$plan.steps.Add('Apply completed (nssm -> full jeeves all cmdline; no BobIrcd depend; FR#72 env secrets/resync)')
            }
        }
    }
}

...
7. MRB PASS closes FR; FAIL one fix PR, FR stays open; only Bob stamps UAT.  
   **FR #92:** seats never merge/MRB their own PRs. PR body needs `Seat: {nick}`.
   Reviewing seat posts required check **`mrb/verdict`** via `tools/post_mrb_verdict.py`
   (full `pytest tests/`, duration ≥ 10 min, reviewer ≠ author). See `docs/mrb-enforcement.md`.  

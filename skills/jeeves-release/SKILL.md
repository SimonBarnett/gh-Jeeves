...
1. G1 green on the release commit (`jeeves-token-less-gate` / full `tests/` CI).
2. Tag the release (`JEEVES_RELEASE_TAG` / git tag). Deploy from the tag with
   `tools/Deploy-BobJeevesRelease.ps1` and/or `jeeves-install-service` +
   `tools/Install-BobJeeves.ps1` (never a hotpatch worktree).
